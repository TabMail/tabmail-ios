...
3. **XcodeGen** — Project file generated from `project.yml`. Do not edit `.xcodeproj` directly. **`TabMail.xcodeproj/project.pbxproj` and `xcshareddata/xcschemes/*` are gitignored** — after `git clone` (or pulling changes that touch `project.yml`), run `xcodegen generate` before opening Xcode. Fastlane lanes auto-regenerate via `ensure_xcodegen`.
