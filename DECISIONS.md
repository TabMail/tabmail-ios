...
**Persistent-failure chain-demotion amendment to decision 1 (2026-07-15; owner-directed).** Decision 1's "a transient failure blocks all later jobs" is amended with a THIRD failure class between transient and authoritative-terminal: **persistent** — a provider failure that is permanent-SHAPED but not authoritative-terminal (e.g. an unrecognized structural REST 400 whose body matches none of the adapter's known terminal shapes). Before this amendment such a failure retried forever at the frontier and wedged the entire global FIFO for every account; the rejected alternative (shipped briefly in an earlier release) dropped all 400s — self-healing but silently discarding intent. The owner chose demotion: *"fail and move the failure to the end of the queue — we'd have to move the whole chain related to the failing message though — so that it lets other moves go through. But also report the error loudly."* Classification stays below `EmailProvider` (Law 5/decision 8): Gmail throws `ProviderError.persistentActionFailure` for an action-path 400 whose body does NOT match the known invalid-label shape (which stays an authoritative no-op); Exchange for an action-path 400 other than `ErrorInvalidIdMalformed` (which stays authoritative — the PATCH setter path now classifies it as such too, consistent with move/token resolution); IMAP has no persistent class (its terminal shapes are already classified; protocol errors stay transient). 401/403/429/5xx/transport are NEVER persistent — they stay plain transient and keep blocking the frontier (auth/throttle heal on their own). The queue reacts generically in ONE gated transaction: it computes the failing op's exact RELATED CHAIN — the transitive closure over same-account active rows whose member sets intersect, including the failing op — and re-inserts the chain at the queue TAIL, preserving `id` and every payload byte (delete + insert; only the rowid position changes), keeping intra-chain relative order, all rows `queued`. Per-message serial order is never violated (a chain executes in original relative order); cross-message reordering within an account is the accepted cost, exactly like a user re-issuing the other actions after the failing one. No retry budget ever drops an intention — the chain retries at the tail on every future drain, ONE provider attempt per drain (a per-drain demoted-id spin guard requeues an already-demoted re-claimed frontier untouched and stops the drain, like transient). The error is reported loudly (ungated `[Queue] ERROR: persistent provider failure — demoted N-op chain …` plus the once-per-drain `[QueueDiag]` deep dump). Undo reconciliation is unaffected: its newest-related scan is rowid-based and a demoted chain's newest member is still its newest. Pinned by `AccountManagerQueueDemotionTests` (demotion/recovery/spin-guard/closure/payload-immutability/undo-vs-demoted) and adapter cells in `GmailProviderMockTests`/`ExchangeProviderMockTests`.
