...
- **[MIS-IOS-009](Companion/Mistakes/Active/MIS-IOS-009-amended-a-hash-pinned-fragment-and-killed-the-verifier.md)** — edited a hash-pinned or byte-frozen GENERATED fragment in place, killed its verifier, and hid every later check behind the `abort`. ⚠️ **TWO pinning conventions:** `{Memory,Decisions}/manifest.tsv` hash the **stripped** body — NEVER re-pin; `ported-manifest.tsv` / `amendments-manifest.tsv` / `Decisions/V3/manifest.tsv` hash the **raw** file — MUST re-pin. ⚠️ a GLOB in a comment aborts it too. **Run verifiers UNPIPED — `ruby … | tail` masks rc in zsh.** (×10)
