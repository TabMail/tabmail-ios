...
// MARK: - Direct AI maintenance retention

@Suite("Direct AI maintenance retention")
struct DirectAIMaintenanceRetentionTests {
    private struct Fixture {
        let pool: DatabasePool
        let directory: URL
    }
    ...
    private func makeFixture() throws -> Fixture {
        ...
        let path = directory.appendingPathComponent("maintenance.sqlite").path
        var configuration = Configuration()
        configuration.foreignKeysEnabled = true
    ...
    private func seedAccountAndFolder(_ pool: DatabasePool) throws -> (Account, Folder) {
    ...
    @Test("Production header-budget pruning removes regular rows and pins direct rows")
    func headerBudgetPruningRetainsPendingArtifacts() throws {
        let fixture = try makeFixture()
        defer {
            TestDatabaseTeardown.closeThenUnlinkNow(
                pool: fixture.pool, directory: fixture.directory)
        }
        let (account, folder) = try seedAccountAndFolder(fixture.pool)
        let now = Date()
        var regularIds: [String] = []
        let pendingId = try fixture.pool.write { db -> String in
            for i in 0..<3 {
                var header = MessageHeader(
                    messageId: "regular-\(i)", subject: "Regular", from: "Sender",
                    fromAddress: "sender@example.com", to: "user@example.com",
                    date: now.addingTimeInterval(Double(i)), snippet: "",
                    folderId: folder.id, accountId: account.id,
                    folderPath: folder.path, isInInbox: true)
                header.rfc822MessageId = "regular-\(i)@example.com"
                try header.insert(db)
                regularIds.append(header.id)
            }
            var pending = MessageHeader(
                messageId: "pending", subject: "Pending", from: "Sender",
                fromAddress: "sender@example.com", to: "user@example.com",
                // Deliberately oldest: without the production pending-row filter,
                // the unfiltered oldest-three batch must delete this row.
                date: now.addingTimeInterval(-10), snippet: "",
                folderId: folder.id, accountId: account.id,
                folderPath: folder.path, isInInbox: true)
            pending.rfc822MessageId = "pending@example.com"
            try pending.insert(db)
            try ActiveAIQueue.markDirectPending(headerIds: [pending.id], db: db)
            return pending.id
        }

        var released: [ContentKey] = []
        let removed = SyncEngine.pruneHeadersWhileOverBudget(
            dbPool: PrioritizedDatabase(pool: fixture.pool, priority: .background),
            accounts: [account], foldersByAccount: [account.id: [folder]],
            floor: 1, pruneChunkSize: 50,
            isOverBudget: { true },
            releaseContent: { released.append(contentsOf: $0) })

        let evidence = try fixture.pool.read { db -> (Int, Int, Bool?) in
            let regular = try MessageHeader
                .filter(regularIds.contains(Column("id"))).fetchCount(db)
            let pending = try MessageHeader
                .filter(Column("id") == pendingId).fetchCount(db)
            let mirror = try Bool.fetchOne(
                db, sql: "SELECT aiDirectPending FROM messageAICache WHERE rfc822MessageId = ?",
                arguments: ["pending@example.com"])
            return (regular, pending, mirror)
        }
        #expect(removed == regularIds.count)
        #expect(Set(released.map(\.rawValue)) == Set(regularIds))
        #expect(evidence.0 == 0)
        #expect(evidence.1 == 1)
        #expect(evidence.2 == true,
                "header pruning must preserve both the live marker and its durable mirror")
    }

    @Test("Production AI-cache TTL purge removes regular entries and pins direct mirrors")
    func cacheTTLPurgeRetainsPendingArtifacts() throws {
        let fixture = try makeFixture()
        defer {
            TestDatabaseTeardown.closeThenUnlinkNow(
                pool: fixture.pool, directory: fixture.directory)
        }
        let expired = try #require(Calendar.current.date(
            byAdding: .day, value: -(SyncConfig.aiCacheTTLDays + 1), to: Date()))
        try fixture.pool.write { db in
            var regular = MessageAICache(
                key: "acc1:Archive:regular@example.com",
                rfc822MessageId: "regular@example.com")
            regular.updatedAt = expired
            try regular.insert(db)

            var pending = MessageAICache(
                key: "acc1:INBOX:pending@example.com",
                rfc822MessageId: "pending@example.com")
            pending.aiDirectPending = true
            pending.updatedAt = expired
            try pending.insert(db)
        }

        let persistedPendingDate = try fixture.pool.read { db in
            try #require(MessageAICache.fetchOne(
                db, key: "acc1:INBOX:pending@example.com")?.updatedAt)
        }

        SyncEngine.runPurgeExpiredAICache(
            dbPool: PrioritizedDatabase(pool: fixture.pool, priority: .background))

        let evidence = try fixture.pool.read { db -> (Bool, Bool, Date?) in
            let regular = try MessageAICache.fetchOne(
                db, key: "acc1:Archive:regular@example.com") != nil
            let pending = try MessageAICache.fetchOne(
                db, key: "acc1:INBOX:pending@example.com")
            return (regular, pending != nil, pending?.updatedAt)
        }
        #expect(!evidence.0)
        #expect(evidence.1)
        #expect(evidence.2 == persistedPendingDate,
                "a pending mirror is excluded from TTL mutation, not merely rescued")
    }
}
