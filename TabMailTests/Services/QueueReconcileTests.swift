...
struct ReconcileCrashRecoveryTests {
    ...
    func cancelledOpsDeleted() throws {
        ...
        var op = makeOp(status: .cancelled)
        try db.write { try op.insert($0) }
    ...
    func mixedStatuses() throws {
        ...
        var opInFlight = makeOp(type: .markRead, status: .inFlight)
        var opCancelled = makeOp(type: .markUnread, status: .cancelled)
...
// MARK: - Suite 4: queueTagWrite stableId logic

@Suite("QueueTagWrite StableId Logic")
struct QueueTagWriteStableIdTests {

    @Test("Numeric messageId + rfc822MessageId → uses rfc822MessageId")
    func numericWithRfc822() {
        let messageId = "12345"
        let rfc822 = "<msg@example.com>"

        let stableId: String
        if UInt32(messageId) != nil, let rfc822 = Optional(rfc822), !rfc822.isEmpty {
            stableId = rfc822
        } else {
            stableId = messageId
        }

        #expect(stableId == rfc822)
    }

    @Test("Non-numeric messageId → uses messageId directly (Gmail)")
    func nonNumericUsesMessageId() {
        let messageId = "gmail_msg_abc123"
        let rfc822 = "<msg@example.com>"
        ...
        #expect(stableId == messageId)
    }

    @Test("Numeric messageId + nil rfc822MessageId → uses messageId")
    func numericWithNilRfc822() {
        let messageId = "99999"
        let rfc822: String? = nil

        let stableId: String
        if UInt32(messageId) != nil, let rfc822, !rfc822.isEmpty {
            stableId = rfc822
        } else {
            stableId = messageId
        }
    ...
    @Test("Numeric messageId + empty rfc822MessageId → uses messageId")
    func numericWithEmptyRfc822() {
        let messageId = "55555"
        let rfc822: String? = ""
    ...
    @Test("setTag op created for non-nil tag")
    func setTagCreated() throws {
        let db = try TestDatabase.make()
        try TestDatabase.insertAccount(db)

        let tag = ActionTag.reply
        let opType: OperationType = tag != nil ? .setTag : .removeTag
        let op = PendingOperation(type: opType, messageIds: ["<msg@ex.com>"], accountId: "acc1", folderPath: "INBOX", tagValue: tag.rawValue)
        try db.write { try op.insert($0) }
    ...
    @Test("removeTag op created for nil tag")
    func removeTagCreated() throws {
        let db = try TestDatabase.make()
        try TestDatabase.insertAccount(db)

        let tag: ActionTag? = nil
        let opType: OperationType = tag != nil ? .setTag : .removeTag
        let op = PendingOperation(type: opType, messageIds: ["<msg@ex.com>"], accountId: "acc1", folderPath: "INBOX", tagValue: nil)
        try db.write { try op.insert($0) }
...
struct DrainAlgorithmTests {
    ...
    func batchSplitOnMessageNotFound() throws {
        ...
        var batchOp = makeOp(type: .move, messageIds: ["msg1", "msg2", "msg3"], destinationPath: "Archive")
    ...
    func singleMessageNotFoundDeletesOp() throws {
        ...
        var op = makeOp(type: .move, messageIds: ["msg1"], destinationPath: "Archive")
        try db.write { try op.insert($0) }
    ...
    @Test("uidResolutionFailed on tag ops → delete (best-effort skip)")
    func uidResolutionFailedTagOpsDeleted() throws {
        let db = try TestDatabase.make()
        try TestDatabase.insertAccount(db)

        var op = makeOp(type: .setTag, messageIds: ["<msg@ex.com>"], tagValue: "reply")
        try db.write { try op.insert($0) }

        // Simulate uidResolutionFailed on tag op — delete (best-effort skip)
        let error = ProviderError.uidResolutionFailed("<msg@ex.com>")
    ...
    @Test("uidResolutionFailed on non-tag ops → reset to queued (will retry)")
    func uidResolutionFailedNonTagResetsToQueued() throws {
        let db = try TestDatabase.make()
        try TestDatabase.insertAccount(db)

        var op = makeOp(type: .move, messageIds: ["<msg@ex.com>"], destinationPath: "Archive", status: .inFlight)
        try db.write { try op.insert($0) }

        // Simulate uidResolutionFailed on non-tag op — reset to queued
        let error = ProviderError.uidResolutionFailed("<msg@ex.com>")
        let isTagOp = [OperationType.setTag, .removeTag].contains(op.type)
        if case ProviderError.uidResolutionFailed = error, !isTagOp {
            try db.write { dbConn in
    ...
    @Test("uidResolutionFailed on removeTag → also deleted (best-effort)")
    func uidResolutionFailedRemoveTagDeleted() throws {
        let db = try TestDatabase.make()
        try TestDatabase.insertAccount(db)

        var op = makeOp(type: .removeTag, messageIds: ["<msg@ex.com>"])
        try db.write { try op.insert($0) }
