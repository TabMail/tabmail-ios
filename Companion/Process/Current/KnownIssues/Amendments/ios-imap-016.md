...
> **2026-09-11 status amendment:** Cocoanetics/SwiftMail#233 was approved and squash-merged upstream as `124e3cc` (2026-09-11). Fork `main` was reset to that commit and force-pushed; `git log upstream/main..main` is EMPTY — the fork is a pure mirror again, no deviation. App and NSE re-pinned to `124e3cca5bea7d544925455533cdbce72d6f9755`; resolved checkout proven by `rev-parse` before and after the build; app and NSE build with only the documented benign App Intents diagnostic; EML consumer suites (EmlParsing, EmlMarker ×2, AttachmentFilenameContainment, EmlRender, GmailProviderEnvelope, EmailRenderPipeline — 175 tests in 7 suites) pass. Completion criteria: (2) satisfied by the approved upstream commit `124e3cc` (no tagged release yet — SwiftMail's latest release remains 1.11.0); (3) fork synchronized; (4) re-pinned, render/parse suites pass — the send/draft suites were not re-run for this docs-and-pin change because #233 touches only the receive-side RFC 2231 decoder; (5) this record. Owner decides whether to close #10 on the approved commit or wait for a tagged release.
>
