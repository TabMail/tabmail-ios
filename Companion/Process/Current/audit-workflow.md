...
0. 🚨 **REVIEWS RUN THROUGH THE `tabmail-review` SKILL — it supersedes every ad-hoc review recipe below (2026-09-01).** One fresh-context reviewer on the OTHER model carries all four angles (architecture, correctness, robustness-security, test-coverage), emitting one countable `ANGLE:` block per angle. **One clean round closes the gate.** A refused review is ABSENT (reframe once, then fall back); quota exhaustion is not a verdict. Canonical: `../../../../.agents/skills/tabmail-review/SKILL.md` (Codex primary) and `../../../../.claude/skills/tabmail-review/SKILL.md` (Claude primary); keep the two in step. Items 4b–4c below still govern exact-candidate pinning and commit hygiene; historical implementation-role language below is superseded by item 4.

4. **Model, effort, role, main-loop, and delegation policy is owned only by the root** [`model-tier-and-agent-role-routing.md`](../../../../Companion/Rules/Active/model-tier-and-agent-role-routing.md). **No rule in this file selects an actor, model, effort, or mandatory implementation subagent.** The older actor/model-specific clauses below are retained as history and must not be executed. This file adds only iOS constraints: never expose signing configuration to an external reviewer; run builds in an authorized checkout; count the one exact benign App Intents diagnostic and no other warning exception.
...
## Preserved predecessor workflow — NOT role/model authority

**HISTORICAL BODY — item 4 in the current routing note supersedes every actor, model, effort, and delegation statement below.** Non-role iOS safety lessons remain binding.
...
2h. **SUPERSEDED — do not select models or effort here.** The root [`model-tier-and-agent-role-routing.md`](../../../../Companion/Rules/Active/model-tier-and-agent-role-routing.md) is the only current authority for both Claude Code and Codex.
