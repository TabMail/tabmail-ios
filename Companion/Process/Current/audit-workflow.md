...
0. 🚨 **REVIEWS RUN THROUGH THE `tabmail-review` SKILL — it supersedes every ad-hoc review recipe below (2026-09-01).** A review round is a four-angle fan-out (architecture, correctness, robustness-security, test-coverage), one fresh-context specialist per angle, and **the specialists always run on the OTHER model**: Codex primary dispatches its angles to Claude (via `$claude-review`'s runner, one unique prompt/report stem per angle); Claude primary dispatches its angles to Codex. **One clean round closes the gate** — the two-consecutive-clean rule is retired for every model in every configuration. A refused angle is ABSENT (reframe once, then run it on the other model); quota exhaustion is not a verdict, just fall back. An `ANGLE_UNCLEAN` verdict does not entitle that angle to a re-run — if the revision did not move what the angle owns, carry its findings forward instead. Canonical: `../../../../.agents/skills/tabmail-review/SKILL.md` (Codex primary) and `../../../../.claude/skills/tabmail-review/SKILL.md` (Claude primary); keep the two in step. Items 1–4 below still govern implementation roles, exact-candidate pinning, and commit hygiene.
