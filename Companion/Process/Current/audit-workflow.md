...
0. 🚨 **REVIEWS RUN THROUGH THE `tabmail-review` SKILL — it supersedes every ad-hoc review recipe below.** One fresh-context **Codex** reviewer carries all assigned angles regardless of the coordinating harness, emitting one countable `ANGLE:` block per angle. **One clean round closes the gate.** A refusal is ABSENT; retry once with defensive framing, then fall back to fresh-context Fable `max` reviewers (owner 2026-09-09); quota exhaustion falls back the same way. Canonical: `../../../../.agents/skills/tabmail-review/SKILL.md` (Codex primary) and `../../../../.claude/skills/tabmail-review/SKILL.md` (Claude primary). Items 4b–4c below still govern exact-candidate pinning and commit hygiene; the frozen heading, the "CURRENT OVERRIDE" paragraph, and items 1, 2, 2a and 2h below are historical text preserved for the verifier and must not be executed — item 4 supersedes every actor, model, effort and delegation statement in them.
