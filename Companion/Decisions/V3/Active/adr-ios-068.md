...
|---|---|
| **`MessageExistenceProbe` / `currentUIDs` / `messageExistsInFolder`** ★★★ | ⚠️ **THIS CELL OPENED WITH *"Two consumers: the queue and, destructively, the backfill body queue"* UNTIL R16-7 (corrected 2026-08-06), AND BOTH HALVES WERE WRONG.** The **action queue is not a consumer of this protocol at all** — naming it here sent a reader auditing the queue's identity contract to a surface it does not touch, and (worse for the KEEP argument below) implied the destructive consumer had a non-destructive sibling holding it honest. The true shape: the protocol is *witnessed* in exactly **one** place, `BackfillBodyQueue.confirmGoneAtThreshold` (`as? any MessageExistenceProbe`, then `prober.currentUIDs`), and that **one destructive function has two callers** — `BackfillBodyQueue` itself and `ActiveBodyQueue` — both body-fetch paths. `messageExistsInFolder` is declared and implemented but has **zero** call sites, so `currentUIDs` alone carries the conformance. Predicate, skipping comments so nothing here can satisfy it: `rg -n --pcre2 '^(?!\s*(///\|//)).*(MessageExistenceProbe\|confirmGoneAtThreshold\|messageExistsInFolder\|currentUIDs)' TabMail/ Shared/ TabMailNotificationService/` → **12** lines: **1 protocol HEADER line** (`protocol MessageExistenceProbe: Sendable {`), 1 conformance, 2 protocol member declarations, 2 IMAP implementations + 1 internal call, 1 `as?` witness, 1 `prober.currentUIDs`, 2 `confirmGoneAtThreshold` callers, 1 `confirmGoneAtThreshold` definition. ⚠️ **THIS CELL SAID 11 AND ITEMISED 11 UNTIL R18-D6 (corrected 2026-08-07); the predicate beside it returned 12 the whole time.** The omitted line is the protocol's own `protocol MessageExistenceProbe: Sendable {` header — the declaration the pattern's FIRST alternative exists to match, dropped from the itemisation because the eye reads *"protocol declarations"* as the two members inside the braces. **The KEEP verdict is untouched**; what is instructive is the failure mode. This is the good case `IOS-DOC-002` clause (b) predicts: the cell shipped its predicate AND an itemisation, so re-running one command and counting the bullets exposed the miss in seconds — a bare *"11"* with no predicate would have stayed wrong indefinitely and kept its credibility. The correction is to the ARITHMETIC of the itemisation, not to any claim about the code. **The KEEP verdict is unchanged and is if anything stronger** — a single witness with no sibling is exactly the shape `feedback_deleting_a_protocol_conformance_flips_a_guard` warns about. `confirmGoneAtThreshold` opens `guard let prober = provider as? any MessageExistenceProbe else { return .gone }`, and `.gone` deletes the user's local header. **Removing the conformance does not disable a check — it converts every IMAP backfill miss at threshold into "delete the user's local header and body"**, and destroys the UID-remap re-key that project memory records as the 2026-06-09 FTS body-loss root cause. **KEEP THE CONFORMANCE.** |
...
| **`AccountManager.AIWriteTarget.resolveCurrentHeader` arm 6 — the AI write-back CONTENT PROOF** (added 2026-08-06, R12-T10) | **Authority-class by this table's own test, and it had no row — which the paragraph above calls a bug in itself.** A non-nil return from `resolveCurrentHeader` is the sole licence for `aiGuardedHeaderWrite`'s `mutate` closure, and arm 6 returns the row on a non-empty `rfc822MessageId` equality **and skips arms 7–8**, i.e. it substitutes for the numbering proof rather than merely declining to veto. It is therefore not a refuse-only witness and is not covered by the carve-out below. It belongs here for the ADR-IOS-072 reason clause 7 states: an AI summary is DERIVED CONTENT, and derived content keys by RFC. The nearest existing row (*AI cross-device cache probe*) names a different symbol and a different mechanism and does not cover it. <br>**Why the remedy is a row and never a sweep:** deleting arm 6 reinstates Leg 1 bullet 3 verbatim — an RFC-less row with no epoch baseline makes `capture` return nil, the whole AI job no-ops, and the message becomes permanently un-analysable while the LLM is re-billed on every open. <br>**The bounded residual, stated rather than implied:** selection is still by the native `headerId` composite (arms 1–3), and arm 5's `guard folder?.uidValidityResetPendingAt == nil else { return nil }` runs first, so to misattribute you would need a **different** message carrying the **same** non-empty Message-ID at the **same** account+folder+native id — an RFC 5322 duplicate, for which the derived summary is correct by construction (ADR-IOS-072). |
| **`AccountManagerQueue.resolveInboxEntryAITargets` — entered-inbox AI target selection** (added 2026-08-15, `IOS-AI-004`) | **Authority-class:** the RFC match positively selects the row whose `headerId` is enqueued; it is not merely a veto. The recorded UID is a pre-sync address and can be stale after either move re-key path, so RFC is the only recorded identity that survives the destination sync. Routing this through `DurableIdentityLookup.find` previously selected an unrelated destination row whose folder-local UID collided with the stale source UID; the call-site warning and `MoveIntoInboxAIEnqueueTests.aiTargetIsNeverAUidCollisionVictim` pin that negative case. RFC-less input therefore remains fail-closed, matching Thunderbird's `getUniqueMessageKey`. <br>**The bounded residual:** duplicate Message-IDs within one account+folder resolve deterministically to the lowest BINARY `id` (`ORDER BY id ASC LIMIT 1`). This is acceptable for this surface because it schedules only DERIVED CONTENT, whose cross-copy key is RFC by ADR-IOS-072; `ActiveAIQueue.enqueue` then rechecks that the selected live row belongs to the bounded recent-Inbox population, and every eventual header mutation is independently re-proved by `AIWriteTarget.resolveCurrentHeader`. <br>⚠️ **SUPERSEDED IN PART 2026-08-19 (ADR-IOS-078 pathway regating, owner directive):** the middle leg no longer runs for THIS caller. `enqueueAIForMembersThatEnteredInbox` now calls `enqueue(…, windowExempt: true)`, so `ActiveAIQueue.enqueue` performs no recent-Inbox recheck on the rows this resolver selects — a move into the Inbox is explicit user intent and is deliberately not bounded by the newest-window flood guard. The other two legs of this acceptance argument STAND unchanged (derived-content-only, whose cross-copy key is RFC per ADR-IOS-072; and the `AIWriteTarget.resolveCurrentHeader` re-proof of every header mutation), and inbox scope is now held by `ActiveAIQueue.executeJob`'s unconditional `message.isInInbox` re-check. The duplicate-Message-ID lowest-binary-`id` residual is therefore still bounded and still accepted; only its stated justification is narrowed. |
