...
2. **The app fetches required parts in one-MiB chunks.** Transfer-encoded bytes are concatenated
   ...
   A BODYSTRUCTURE size is the preferred termination authority; where size is unavailable, a short
   nonempty response is not treated as EOF and fetching continues until an empty range. The
   server-reported size does not trigger a full-size allocation before bytes arrive.
...
4. **The ordinary four-MiB response parser limit and NSE memory policy stay unchanged.** The bounded
   transport is the fix; there is no larger-buffer fallback. The NSE uses the same payload selection
   and chunking and never downloads ordinary attachment payloads.
5. **A server that cannot honor partial ranges creates an explicit terminal-unindexed state.**
   ...
   retryable. The write is guarded by the row's full provider address so a move/re-key cannot attach
   the outcome to another message.
...
- Normal attachment payloads are absent from background `MessagePart` values and are downloaded
  only when requested. BODYSTRUCTURE still supplies their filename, MIME type, section and size.
...
assembly, attachment selection, unknown-size termination, migration/default state, queue exclusion,
Smart Reindex recovery, and exact completion wording.
