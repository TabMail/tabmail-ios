...
final class InboxViewModel {
    ...
    func lookupMessage(_ id: String) -> MessageHeader? {
        ...
        let currentId = MessageHeaderRekey.currentHeaderId(afterHandoffFrom: id)
        if currentId != id,
           let header = try? dbPool.read({ db in
               try MessageHeader.fetchOne(db, key: currentId)
           }) {
            #if DEBUG
            BackgroundSyncLogger.logInbox(
                "[RoleActionTrace] vm lookup phase=followProviderRekey "
                    + "old=\(id) current=\(currentId)")
            #endif
            return header
        }
        ...
        return NSEDataBridge.latestStagedRows.withLock { rows in
            rows.first { $0.headerId == currentId }
                ?? (currentId == id ? nil : rows.first { $0.headerId == id })
        }?.toMessageHeader()
    }
    ...
    private func overlayAdjustedForAction(_ header: MessageHeader) -> MessageHeader {
        guard let overlay = manager.snapshotOverlayMutation(
            forCurrentHeaderId: header.id)
        else { return header }
        let mutation = overlay.mutation
        #if DEBUG
        if overlay.id != header.id {
            BackgroundSyncLogger.logInbox(
                "[RoleActionTrace] vm overlay phase=followProviderRekey "
                    + "old=\(overlay.id) current=\(header.id)")
        }
        #endif
        var adjusted = header
    ...
    /// DEBUG-only trace for the rare slow-IMAP window where a visible row and
    /// its durable/re-keyed address disagree. Never logs subject, sender, body,
    /// or RFC 822 identity; the header id and folder locations are sufficient
    /// to correlate a tap with Undo, overlay lifetime, and queue admission.
    private func logRoleActionLocation(
        action: String,
        phase: String,
        stored: MessageHeader?,
        effective: MessageHeader?,
        destinationPath: String? = nil,
        decision: String
    ) {
        guard DebugModeManager.isLoggingEnabled() else { return }
        func describe(_ header: MessageHeader?) -> String {
            guard let header else { return "<missing>" }
            let role = lookupFolderRole(header.folderId)?.rawValue ?? "<unknown>"
            return "id=\(header.id) folderId=\(header.folderId) "
                + "folderPath=\(header.folderPath) role=\(role) "
                + "isInInbox=\(header.isInInbox)"
        }
        let id = stored?.id ?? effective?.id ?? "<missing>"
        let destination = destinationPath ?? "<unresolved>"
        BackgroundSyncLogger.logInbox(
            "[RoleActionTrace] vm action=\(action) phase=\(phase) "
                + "decision=\(decision) destination=\(destination) "
                + "stored={\(describe(stored))} effective={\(describe(effective))} "
                + manager.roleActionOverlayDiagnostic(id: id))
    }
        ...
        for messageId in ids {
            guard let idx = loadedMessages.firstIndex(where: { $0.id == messageId }) else {
                if DebugModeManager.isLoggingEnabled() {
                    BackgroundSyncLogger.logDebug("[AIBatchTrace] skipped id absent from loaded snapshot: \(messageId)")
                }
                continue
            }
            guard let header = freshHeaders[messageId] else {
                if DebugModeManager.isLoggingEnabled() {
                    BackgroundSyncLogger.logDebug("[AIBatchTrace] skipped id absent from durable headers: \(messageId)")
                }
                continue
            }
    ...
    func reloadMessages(animated: Bool = false) async {
        ...
        if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[MoveTrace] reloadMessages — newCount=\(loadedMessages.count) hasMore=\(hasMoreMessages)") }
    ...
    private func applyResolvedFolders(_ resolved: [Folder]) {
        ...
        if currentKeys != resolvedKeys {
            ...
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[MoveTrace] self-healed folders from GRDB: \(currentKeys.count) → \(resolvedKeys.count)") }
            folders = resolved
        }
    }
    ...
    func loadMoreMessages() {
        ...
        Task { @MainActor in
            ...
            do {
                ...
                if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[InfiniteScroll] Error: \(error)") }
    ...
    private func queueSnippetIfNeeded(_ id: String) -> Bool {
        ...
        guard !inFlight, !failed, !queued else {
            if failed {
                if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] BLOCKED by snippetFailed: \(id.prefix(40))") }
            }
            return false
        }
    ...
    private func loadSnippetBatch() async {
        ...
        if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Batch \(batch.count): tier0=\(snippetUpdates.count) needsFTS=\(needsFTS.count) readMs=\(readMs)") }
        ...
        for headerId in needsFTS {
            ...
            if let header = headerFor(headerId) {
                ...
                if header.isBodyQuarantined {
                    if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Skipping oversized-quarantined \(headerId.prefix(40))") }
        ...
        if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Tier1 done: ftsHits=\(snippetUpdates.count - batch.count + needsFTS.count) networkNeeded=\(networkNeeded.count)") }

        // Tier 2: Network body fetch — extract snippet + update FTS, release body immediately.
        for item in networkNeeded {
            guard !Task.isCancelled else {
                if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Tier2 cancelled") }
                return
            }
            guard let account = try? await dbPool.read({ db in try Account.fetchOne(db, key: item.header.accountId) }),
                ...
                if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Tier2 NO PROVIDER for \(item.header.accountId) folder=\(item.header.folderPath)") }
                // Provider temporarily unavailable (reconnecting) — don't blacklist, leave retryable
                continue
            }

            do {
                if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Tier2 fetching msgId=\(item.header.messageId) folder=\(item.header.folderPath)") }
                ...
                if case .success(let fetchResult) = await BodyFetchProcessor.renderFetched(item: processorItem, fullMessage: fullMessage) {
                    ...
                    if let processed {
                        ...
                        if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Tier2 no body content for msgId=\(item.header.messageId)") }
                    }
                }
            } catch {
                if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[SnippetLoader] Failed for \(item.headerId): \(error)") }
    ...
    private func performSync() async {
        ...
        if folders.isEmpty {
            ...
            if !resolved.isEmpty {
                folders = resolved
                if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Sync] performSync self-healed folders from GRDB: \(resolved.count)") }
                resetMessages()
            }
        }
        ...
        do {
            ...
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Sync] Error fetching accounts: \(error)") }
            return
        }
        ...
        do {
            ...
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Sync] Transient error (not surfaced): \(error)") }
            ...
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Sync] Database suspended (not surfaced): \(error)") }
        } catch {
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Sync] Error: \(error)") }
    ...
    func archiveIsNoOp(_ messageId: String) -> Bool {
        guard let stored = lookupMessage(messageId) else {
            logRoleActionLocation(
                action: "archive", phase: "guard", stored: nil, effective: nil,
                decision: "allow.lookupMissing")
            return false
        }
        let message = overlayAdjustedForAction(stored)
        if lookupFolderRole(message.folderId) == .archive {
            logRoleActionLocation(
                action: "archive", phase: "guard", stored: stored, effective: message,
                decision: "suppress.effectiveRole")
            return true
        }
        guard let archivePath = lookupFolderPath(
            accountId: message.accountId, role: .archive
        ) else {
            logRoleActionLocation(
                action: "archive", phase: "guard", stored: stored, effective: message,
                decision: "allow.destinationMissing")
            return false
        }
        let isNoOp = message.folderPath == archivePath
        logRoleActionLocation(
            action: "archive", phase: "guard", stored: stored, effective: message,
            destinationPath: archivePath,
            decision: isNoOp ? "suppress.effectivePath" : "allow")
        return isNoOp
    }
    ...
    func deleteIsNoOp(_ messageId: String) -> Bool {
        guard let stored = lookupMessage(messageId) else {
            logRoleActionLocation(
                action: "delete", phase: "guard", stored: nil, effective: nil,
                decision: "allow.lookupMissing")
            return false
        }
        let message = overlayAdjustedForAction(stored)
        if lookupFolderRole(message.folderId) == .trash {
            logRoleActionLocation(
                action: "delete", phase: "guard", stored: stored, effective: message,
                decision: "suppress.effectiveRole")
            return true
        }
        guard let trashPath = lookupFolderPath(
            accountId: message.accountId, role: .trash
        ) else {
            logRoleActionLocation(
                action: "delete", phase: "guard", stored: stored, effective: message,
                decision: "allow.destinationMissing")
        ...
        let isNoOp = message.folderPath == trashPath
        logRoleActionLocation(
            action: "delete", phase: "guard", stored: stored, effective: message,
    ...
    func archive(_ messageId: String) -> Bool {
        ...
        guard let archivePath = lookupFolderPath(accountId: message.accountId, role: .archive) else {
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Queue] ERROR: no archive folder for account \(message.accountId)") }
            return false
        }
        ...
        manager.retainOverlayEntry(id: actionId)
        ...
        manager.registerMutation(id: actionId, mutation: .init(
        ...
        manager.enqueueWriteFromSynchronousContext { [manager] in
            ...
            await manager.markReadBeforeRoleMove(ids: [actionId])
            await manager.move([message], to: archivePath)
            manager.releaseOverlayEntry(id: actionId)
        }
        return true
    }
    ...
    func archiveThread(_ messageIds: [String]) -> [String] {
        let resolved = messageIds.compactMap { requestedId in
            lookupMessage(requestedId).map {
        ...
        guard let archivePath = lookupFolderPath(accountId: first.accountId, role: .archive) else {
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Queue] ERROR: no archive folder for account \(first.accountId)") }
            return messageIds
        }
    ...
    func delete(_ messageId: String) async -> Bool {
        guard let stored = lookupMessage(messageId) else {
            logRoleActionLocation(
                action: "delete", phase: "admission", stored: nil, effective: nil,
                decision: "refuse.lookupMissing")
            return false
        }
        let message = overlayAdjustedForAction(stored)
        let actionId = message.id
        ...
        if folderRole == .drafts {
            logRoleActionLocation(
                ...
                effective: message, decision: "route.draft")
        ...
        if folderRole == .trash {
            logRoleActionLocation(
                action: "delete", phase: "admission", stored: stored,
                effective: message, decision: "refuse.effectiveRole")
            return false
        }
        guard let trashPath = lookupFolderPath(accountId: message.accountId, role: .trash) else {
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Queue] ERROR: no trash folder for account \(message.accountId)") }
            logRoleActionLocation(
                action: "delete", phase: "admission", stored: stored,
                effective: message, decision: "refuse.destinationMissing")
            return false
        }
        guard message.folderPath != trashPath else {
            logRoleActionLocation(
                action: "delete", phase: "admission", stored: stored,
                effective: message, destinationPath: trashPath,
                decision: "refuse.effectivePath")
            return false
        }
        logRoleActionLocation(
            action: "delete", phase: "admission", stored: stored,
        ...
        manager.registerMutation(id: actionId, mutation: .init(
            ...
            actionTag: message.isInInbox ? .some(nil) : nil))
        BackgroundSyncLogger.logInbox(
            "[RoleActionTrace] vm action=delete phase=overlayRecorded id=\(actionId) "
                + manager.roleActionOverlayDiagnostic(id: actionId))
        UndoService.shared.push(UndoableAction(
        ...
        manager.enqueueWriteFromSynchronousContext { [manager] in
            BackgroundSyncLogger.logInbox(
                "[RoleActionTrace] vm action=delete phase=queuedClosure.begin "
                    + "id=\(actionId) source=\(message.folderPath) "
                    + "destination=\(trashPath) "
                    + manager.roleActionOverlayDiagnostic(id: actionId))
            // Read intent BEFORE the move — see `archive(_:)`.
            await manager.markReadBeforeRoleMove(ids: [actionId])
            let admission = await manager.move([message], to: trashPath)
            BackgroundSyncLogger.logInbox(
                "[RoleActionTrace] vm action=delete phase=queuedClosure.admission "
                    + "id=\(actionId) admitted=\(admission.admittedIds.contains(actionId)) "
                    + "pending=\(admission.pendingIds.contains(actionId)) "
                    + "failed=\(admission.failedIds.contains(actionId)) "
                    + manager.roleActionOverlayDiagnostic(id: actionId))
            manager.releaseOverlayEntry(id: actionId)
            BackgroundSyncLogger.logInbox(
                "[RoleActionTrace] vm action=delete phase=queuedClosure.end "
                    + "id=\(actionId) "
                    + manager.roleActionOverlayDiagnostic(id: actionId))
        }
        BackgroundSyncLogger.logInbox(
            "[RoleActionTrace] vm action=delete phase=return id=\(actionId) result=true")
        return true
    }
    ...
    func deleteThread(_ messageIds: [String]) async -> [String] {
        ...
        if folderRole == .drafts {
            ...
            for item in resolved {
                if await deleteDraftMessage(item.message) {
                    settled.insert(item.requestedId)
                }
            }
        ...
        guard let trashPath = lookupFolderPath(accountId: first.accountId, role: .trash) else {
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[Queue] ERROR: no trash folder for account \(first.accountId)") }
            return messageIds
        }
    ...
    func removeUserLabel(_ label: UserLabel, from snapshot: MessageSnapshot) async {
        ...
        if let idx = loadedMessages.firstIndex(where: { $0.id == actionId }) {
        ...
        do {
            let admitted = try await AppDatabase.dbPool.write { db -> Bool in
                ...
                try MessageUserLabel
                    .filter(Column("messageId") == actionId && Column("userLabelId") == label.id)
                    .deleteAll(db)
            ...
            reconcileUserLabels(forMessageId: actionId)
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[InboxViewModel] removeUserLabel failed: \(error)") }
        }
    }
    ...
    private func reconcileUserLabels(forMessageId id: String) {
        ...
        do {
            ...
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[InboxViewModel] reconcileUserLabels failed for \(id): \(error)") }
        }
    }
    ...
    func move(_ messageId: String, toFolderPath: String) -> Bool {
        guard let message = lookupMessage(messageId) else {
            if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[MoveTrace] ViewModel.move — lookupMessage FAILED for id=\(messageId)") }
            return false
        }
        guard lookupFolderRole(message.folderId) != .drafts else { return false }
        let actionId = message.id
        if DebugModeManager.isLoggingEnabled() { BackgroundSyncLogger.logDebug("[MoveTrace] ViewModel.move — msgId=\(message.messageId) from=\(message.folderPath) to=\(toFolderPath) folderId=\(message.folderId) headerDbId=\(message.id)") }
        ...
        manager.registerMutation(id: actionId, mutation: .init(folderId: destFolderId, actionTag: (message.isInInbox && destRole != .inbox) ? .some(nil) : nil))
        UndoService.shared.push(UndoableAction(
    ...
    func moveThread(_ messageIds: [String], toFolderPath: String) -> [String] {
        ...
        guard let first = messages.first else {
            if DebugModeManager.isLoggingEnabled() {
                BackgroundSyncLogger.logDebug("[MoveTrace] ViewModel.moveThread — no messages resolved for ids=\(messageIds)")
            }
            return messageIds
        }
        if DebugModeManager.isLoggingEnabled() {
            BackgroundSyncLogger.logDebug("[MoveTrace] ViewModel.moveThread — count=\(messages.count) from=\(first.folderPath) to=\(toFolderPath)")
        }
