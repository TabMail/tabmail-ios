...
/// Preview: `ArchiveOldEmailsTip` in `InboxView` — appears when inbox has old messages.
/// `spotlight` bypasses the `isLargeInbox` rule.
#Preview("ArchiveOldEmailsTip — InboxView") {
    let scenario = PreviewMocks.seedInbox()
    PreviewMocks.spotlight(ArchiveOldEmailsTip.self)
