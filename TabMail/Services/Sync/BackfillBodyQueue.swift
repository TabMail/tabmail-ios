            ...
            let items: [Item] = try await dbPool.read { db in
                try Row.fetchAll(db, sql: """
                    ...
                    WHERE headerComplete = 1 AND bodyComplete = 0 AND bodyEmptyConfirmed = 0
                      AND bodyIndexingFailureReason IS NULL AND isInInbox = 0
                    ORDER BY date DESC
                    """)
                .map { row in
                    Item(
                ...
                do {
                    ...
                    if let providerError = error as? ProviderError,
                       ...
                       let failedItem = items.first(where: { $0.messageId == messageId }) {
                        ...
                        print("[BackfillBody] Partial fetch unsupported — \(terminalized ? "recorded terminal-unindexed state" : "row changed; retrying")")
