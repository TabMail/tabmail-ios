...
- **REAL root cause of "big email renders slow / sized-but-empty box": remote `<img>`s block WebKit's first paint — DEFER them.** WebKit (`loadHTMLString`) holds the first compositor frame until `readyState=complete`, which waits on EVERY pending subresource. Vancouver Sun newsletter: `IMAGE AUDIT total=28 pending=28`, `first rAF`==`readyState=complete`==+2732ms → 2.7s blank even though the frame was sized at +196ms. (The earlier render-blocking-CSS theory was WRONG — `unwrapFullHTMLDocument` already strips the `<head>` so the Google Fonts `<link>` never reached the doc; the stylesheet strip logged `removedChars=0`. Keep the strip anyway: harmless + covers body-level/`.eml` links + privacy.) **Fix = BLOCK remote content with a tap-to-load banner (TB-style privacy default; user chose this over auto-defer, 2026-06-17).** `EmailHTMLWrapper.wrapHTML` rewrites remote `http(s)` `<img src/srcset>` → `data-tmsrc/data-tmsrcset` (cid:/data:/local-scheme untouched) so NOTHING loads on open → zero pending image loads → `complete` fires at ~DOMContentLoaded → text/layout paints immediately, AND tracking pixels (`li.postmedia.com/imp?...&e=<email>&stpe=pixel`) never fire. When anything was blocked it injects `#tm-remote-banner` ("To protect your privacy, remote content has been blocked. Tap to load.") as the first body child (light/dark CSS in the wrapper). `remoteContentBannerJS` (atDocumentEnd WKUserScript, NOT debug-gated) attaches a click listener that restores the real URLs and removes the banner — load happens ONLY on tap (no auto-swap/timeout). No remote content → no banner (no-op). Frame grows via the ResizeObserver when images load post-tap. Known gap: CSS `background-image:url(http…)` is NOT yet blocked (only `<img>` src/srcset); fine for the reported trackers, revisit if needed. Tests: `wrapperBlocksRemoteImagesWithBanner`, `wrapperNoBannerWithoutRemoteContent`, `remoteContentBannerScript`. **Debugging aids that cracked this (keep, debug-gated):** `[RENDERER BUILD] rev=…` marker (proves which code is live — bump `rev` per change), `[RENDERER] stylesheet-link strip/deferred N` logs, and the `[Ttrace +Nms]` lifecycle timeline (`documentStart`/`first rAF`/`fonts.ready`/`readyState`) + `[FitReq/FrameApply +Nms]`. KEY LESSON: WebKit `loadHTMLString` does NOT paint progressively — first paint waits for `readyState=complete` (all subresources), so ANY slow remote subresource = blank email until it resolves.
