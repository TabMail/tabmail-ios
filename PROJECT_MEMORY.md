...
| Current | 🚨 **"TRIAL ENDED" IS DERIVED, NEVER A NEW `/whoami` FLAG** — `has_subscription:false` + past `trial.trial_end`; `trial_expired`/`trial_blocked` were CUT as reinvention. `AccountInfo.trialState(now:)`, `TrialState.noTrial` (not `.none`), `AISubscriptionGate.apply`/`trialHasEnded` (bare `openGate`/`closeGate` must NOT write it), `ZeroBudgetPlan` per-plan quota captions, `displayPlanName` `Trial → "Free Trial"`, signup trial, plan picker, StoreKit intro offer self-extinguishes | [read in full](Companion/Memory/Current/118-trial-ended-is-derived-never-a-new-whoami-flag.md) |
