...
| Current | `isDeletedOnServer` — FOUR materialisation paths (`selectStaleHeaders`, `runSyncMessages`, `insertBackfillBatchGuardable` funnel, `fetchOlderMessages`); crawls advance on `found`, never `inserted`; infinite-scroll exhaustion / `hasMoreMessages` / `mayHaveMore` is SERVER COVERAGE + progress, never materialised rows | [read in full](Companion/Memory/Current/101-isdeletedonserver-has-four-materialisation-paths.md) |
