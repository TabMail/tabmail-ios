...
- Gmail attachment metadata extracted in `fetchMessage` via `extractAttachments(from:)` — recursively walks `GmailPart` tree for parts with non-empty `filename` and `body.attachmentId`
