...
| Current | ⚠️ **FIVE irreversible wire operations, not one** — `COPYUID`-gated source expunge, the **draft family** (`deleteDraftStrong`, `saveDraft`, `GmailProvider.deleteDraft`), `CalDAVProvider.deleteEvent`; plus the **replacement family** (`splitSeries`'s cap `PUT`). Enumerate BOTH spellings (`httpMethod =` / `method:`). ⚠️ **`CLAUDE.md` § THE MANTRA is the STALE copy** — it states the set by VERB and omits the replacement family, so the ROUTED FILE is the authority until the owner lands the handoff edit (re-verified round-18 D4; re-reported as a "new" finding because the stale copy is what auditors read) | [read in full](Companion/Memory/Current/102-there-are-four-irreversible-wire-operations-not-one.md) |
...
| Current | 🚨 **BOTH UIDVALIDITY RE-DRIVE OWNERS ITERATED `syncableFolders`** (`fullSync` + `imapDeltaSync`), so an armed CUSTOM NON-FAVOURITE folder had NO re-drive — quarantined forever, mail purged; 26 folders / 145,754 rows (85%) on the reference device. `syncFolderMessages` is now the third owner. Count PREDICATES, not call sites | [read in full](Companion/Memory/Current/114-both-uidvalidity-redrive-owners-iterate-syncablefolders.md) |
